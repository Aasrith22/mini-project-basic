...
def get_tech_data():
    try:
        ...
        if category not in TECH_COMPANIES or company not in TECH_COMPANIES[category]:
            return jsonify({"error": "Invalid category or company"}), 400
        ...
        for date, row in hist.iterrows():
            data_point = {
                ...
                'volatility': float(row['High'] - row['Low'])
            }
            data.append(data_point)
        
        data_store.tech_data = data
        logger.info(f"Successfully collected {len(data)} tech data points")
        return jsonify(data)
    except Exception as e:
